...
class GPUPerfConvolutionBridgeTest_paper3a_CPU_4GPU : public ::testing::Test {
  public:
    typedef typename TypeParam::T T;
    GPUPerfConvolutionBridgeTest_paper3a_CPU_4GPU(){
      ...
      layer_param.set_gpu_0_batch_proportion(0.22);
      layer_param.set_gpu_1_batch_proportion(0.22);
      layer_param.set_gpu_2_batch_proportion(0.22);
      layer_param.set_gpu_3_batch_proportion(0.22);
      ...
      ParallelizedConvolutionBridge_ = new ParallelizedBridge<DataType_SFFloat,
              ConvolutionBridge>(layer1,
                  layer2, &layer_param, &solver_param, &pdriver, 28, 1);

      ParallelizedConvolutionBridge_->needs_to_calc_backward_grad = false; // like Caffe
    }

    virtual ~GPUPerfConvolutionBridgeTest_paper3a_CPU_4GPU() {
		delete layer1;
		delete layer2;
...
//    static const int mB = 1024;
//    static const int iD = 96;
//    static const int oD = 256;
...
TYPED_TEST_CASE(GPUPerfConvolutionBridgeTest_paper3a_CPU_4GPU, DataTypes);

TYPED_TEST(GPUPerfConvolutionBridgeTest_paper3a_CPU_4GPU, TestForwardBackward){
