...
Kernel<DataType, Layout_CRDB, DataType, Layout_CRDB, DataType, Layout_CRDB, Kernel_GEMM_OpenBlas,
  KERNELCONFIG, DriverClass>::Kernel(const Input1LogicalCubeType * const p_input1_cube,
      const Input2LogicalCubeType * const p_input2_cube, const OutputLogicalCubeType * const p_output_cube,
...
template <typename DataType, KernelConfig KERNELCONFIG, typename DriverClass>
void Kernel<DataType, Layout_CRDB, DataType, Layout_CRDB, DataType, Layout_CRDB, Kernel_GEMM_OpenBlas,
...
template <typename DataType, KernelConfig KERNELCONFIG, typename DriverClass>
Kernel<DataType, Layout_CRDB, DataType, Layout_CRDB, DataType, Layout_CRDB, Kernel_ELEMENTWISEMUL_CPU,
  KERNELCONFIG, DriverClass>::Kernel(const Input1LogicalCubeType * const p_input1_cube,
      const Input2LogicalCubeType * const p_input2_cube, const OutputLogicalCubeType * const p_output_cube,
      DriverClass * _p_driver) :
...
template <typename DataType, KernelConfig KERNELCONFIG, typename DriverClass>
void Kernel<DataType, Layout_CRDB, DataType, Layout_CRDB, DataType, Layout_CRDB, Kernel_ELEMENTWISEMUL_CPU,
     KERNELCONFIG, DriverClass>::compute(const Input1LogicalCubeType * const p_input1_cube,
         const Input2LogicalCubeType * const p_input2_cube, OutputLogicalCubeType * const p_output_cube) {
  report_last_lowering.reset();

  DeviceMemoryPointer * input1 = p_input1_cube->get_device_pointer(p_driver);
  DeviceMemoryPointer * input2 = p_input2_cube->get_device_pointer(p_driver);
  ...
  if (KERNELCONFIG == KernelConfig_NONE) {
    ...
    p_driver->template selementwise_reduce2<_f_reduce_mul>(output, input1, input2, pdummy);
  } else if (KernelConfig_TANHGRAD_ON_INPUT1) {
    //const auto func = [](DataType a, DataType b)->DataType{return (1-a*a)*b;};
    p_driver->template selementwise_reduce2<_f_reduce_tanhgrad>(output, input1, input2, pdummy);
  } else {
