//
//  GeneralConcatBridge.h
//
//  Copyright (c) 2015 Hazy Research. All rights reserved.
//
//  Description:  Like concat bridge but concatenates arbitrary depth layers.
//                Eventually this will replace concat bridge but first need to
//                make sure this one is not any slower than that, since that is
//                used for multi-GPU model parallelism where a few ms can matter.

#ifndef _GeneralConcat_Bridge_h
#define _GeneralConcat_Bridge_h
...
class GeneralConcatBridge : public AbstractBridge<InputLayerDataType, InputLayerLayout, OutputLayerDataType,
  OutputLayerLayout, DriverClass> {
  public:
    typedef Layer<InputLayerDataType, InputLayerLayout> InputLayerType;
    typedef Layer<OutputLayerDataType, OutputLayerLayout> OutputLayerType;

    GeneralConcatBridge(InputLayerType * const _p_input_layer, OutputLayerType * const _p_output_layer,
...
class GeneralConcatBridge<DataType, Layout_CRDB, DataType, Layout_CRDB, DriverClass> : public AbstractBridge<DataType,
...
#include "GeneralConcatBridge_impl.hxx"
