...
#ifndef moka_ParallelizedBridge2_impl2_hxx
#define moka_ParallelizedBridge2_impl2_hxx


// common initialization code, called by both constructors
template<typename DataType, typename BridgeType>
void ParallelizedBridge2<DataType, BridgeType>::initialize() {
...
template<typename DataType, typename BridgeType>
ParallelizedBridge2<DataType, BridgeType>::ParallelizedBridge2(Layer<DataType,Layout_CRDB> * const _input_layer,
...
void exec(PhysicalOperator * op, int phase){
  if(phase == 1){
    op->forward_phase1();
  }else{
    op->forward_phase2();
...
template<typename DataType, typename BridgeType>
void ParallelizedBridge2<DataType, BridgeType>::forward() {
  ...
  //for(int i=0;i<stratum.executors.size();i++){
    //stratum.executors[i]->forward_phase1();
    //stratum.executors[i]->forward_phase2();
  //}

  std::thread t0_1(exec, stratum.executors[0], 1);
  t0_1.join();
  std::thread t0_2(exec, stratum.executors[0], 2);
  std::thread t1_1(exec, stratum.executors[1], 1);
  t0_2.join();
  t1_1.join();
  std::thread t1_2(exec, stratum.executors[1], 2);
  t1_2.join();
  ...
  */
  ...
  {
    #pragma omp single
    stratum.executors[0]->forward_phase1();

    for(int i=0;i<stratum.executors.size();i++){
      #pragma omp single nowait
      {
        if(i != stratum.executors.size() - 1){
          stratum.executors[i+1]->forward_phase1();
        }
      }

      #pragma omp single schedule(dynamic)
      {
        stratum.executors[i]->forward_phase2();
      }
    }
...
template<typename DataType, typename BridgeType>
void ParallelizedBridge2<DataType, BridgeType>::backward() {
  report_backward_updateweight_last_transfer.reset();
  stratum.backward();
